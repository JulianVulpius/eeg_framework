...
const saveRecord = async () => {
  ...
  try {
    if (crud.isEditing.value) await api.put(`category/${props.tableName}/${crud.editingId.value}/`, formData.value)
    else await api.post(`category/${props.tableName}/`, formData.value)
    crud.closeDialog()
    loadData()
  } catch (error) { 
