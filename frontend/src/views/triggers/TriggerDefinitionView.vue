      ...
      <table class="data-table">
        <thead>
          <tr>
            ...
            <th style="width: 45%;">
              <ColumnHeaderFilter :title="$t('views.triggers.trigger_character')" v-model="columnFilters.trigger_character" :placeholder="$t('common.search')" />
            </th>
    ...
    >
      <form @submit.prevent="saveRecord">
        ...
        <div class="form-group">
          ...
          <input type="text" v-model="formData.trigger_character" class="form-control" :class="{ 'input-invalid': crud.fieldErrors.value.trigger_character }" placeholder="e.g., 255, S1, Space" />
...
const columnFilters = ref({ name: '', trigger_character: '' })

const filteredItems = computed(() => {
  return items.value.filter(item => {
    if (columnFilters.value.name && !item.name.toLowerCase().includes(columnFilters.value.name.toLowerCase())) return false
    if (columnFilters.value.trigger_character && (!item.trigger_character || !item.trigger_character.toLowerCase().includes(columnFilters.value.trigger_character.toLowerCase()))) return false
    return true
  })
})
...
const resetForm = () => { formData.value = { name: '', trigger_character: '' } }
const populateForm = (item) => { formData.value = { name: item.name, trigger_character: item.trigger_character } }
...
const saveRecord = async () => {
  ...
  if (!formData.value.trigger_character || formData.value.trigger_character.trim() === '') {
    crud.fieldErrors.value.trigger_character = t('errors.required_field'); hasErrors = true
  }
  ...
  try {
    if (crud.isEditing.value) {
      await api.put(`triggers/definitions/${crud.editingId.value}/`, formData.value)
    } else {
      await api.post('triggers/definitions/', formData.value)
    }
    crud.closeDialog()
    loadData()
  } catch (error) {
...
const confirmAndDelete = (id) => {
  requireConfirmation(async () => {
    try {
      await api.delete(`triggers/definitions/${id}/`)
      loadData()
    } catch (error) {}
  })
}
